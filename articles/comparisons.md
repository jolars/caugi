...
[^3]: `caugi` accepts `class = "AG"` (ancestral graph) in the
    ...
    [`is_mag()`](https://caugi.org/reference/is_mag.md) to validate
...
[^4]: igraph has no dedicated DAG class.
    [`is_dag()`](https://caugi.org/reference/is_dag.md) tests whether a
    directed graph is acyclic, but the package’s single `igraph` object
    class does not enforce DAG invariants. A directed graph that happens
    to be acyclic is representable, but the type is not distinguished at
    the object level.
...
[^11]: gRbase provides a `dag()` constructor and
    [`is_dag()`](https://caugi.org/reference/is_dag.md) type-check, with
    the underlying representation being an igraph object or adjacency
    matrix. There is no dedicated DAG class enforcing acyclicity
    invariants at the object level.

[^12]: Same reasoning as `[^grbase-dag]`: `ug()` constructs an
    undirected graph and
    [`is_ug()`](https://caugi.org/reference/is_ug.md) tests for it, but
    the result is a plain igraph or matrix.
...
[^20]: [`skeleton()`](https://caugi.org/reference/skeleton.md) produces
    a fully undirected graph and `valid.ug()` validates whether a `bn`
    object is completely undirected, but the result is still a plain
    `bn` object—no dedicated UG class with enforced undirected
    invariants exists.
...
[^43]: Parents and children are obtainable via
    ...
    dedicated [`parents()`](https://caugi.org/reference/parents.md) /
    [`children()`](https://caugi.org/reference/children.md) exported
    functions.

[^44]: Ancestors and descendants are obtainable via
    `subcomponent(graph, v, mode = "in" / "out")`, but there are no
    dedicated [`ancestors()`](https://caugi.org/reference/ancestors.md)
    / [`descendants()`](https://caugi.org/reference/descendants.md)
    exported functions.

[^45]: No dedicated
    [`parents()`](https://caugi.org/reference/parents.md) or
    [`children()`](https://caugi.org/reference/children.md) functions.
    `inEdges()` returns the set of in-neighbours (functional equivalent
    of parents on a directed graph);
    [`edges()`](https://caugi.org/reference/edges.md) returns
...
[^46]: `acc()` returns all nodes reachable from a given node on a
    ...
    [`ancestors()`](https://caugi.org/reference/ancestors.md) equivalent
...
[^48]: [`parents()`](https://caugi.org/reference/parents.md) and
    [`children()`](https://caugi.org/reference/children.md) are exported
    (`R/graph-querygraph.R`). There is no corresponding
    [`descendants()`](https://caugi.org/reference/descendants.md)
...
[^49]: [`ancestors()`](https://caugi.org/reference/ancestors.md),
    `ancestralSet()`, and `ancestralGraph()` are exported. A dedicated
    [`descendants()`](https://caugi.org/reference/descendants.md) is
...
[^50]: `searchAM(amat, x, type = "pa" / "ch")` returns parents and
    ...
    no standalone [`parents()`](https://caugi.org/reference/parents.md)
    or [`children()`](https://caugi.org/reference/children.md) function;
    retrieval goes through the general `searchAM()` dispatcher.
...
[^55]: The [`skeleton()`](https://caugi.org/reference/skeleton.md)
...
[^59]: `pa()` returns parents and `ch()` returns children of a node set
    ...
    [`descendants()`](https://caugi.org/reference/descendants.md) or
    [`spouses()`](https://caugi.org/reference/spouses.md) analogue is
    exported.
...
[^65]: `DiGraph.predecessors()` and `DiGraph.successors()` return
    ...
    [`children()`](https://caugi.org/reference/children.md) API for a
    named causal DAG.
...
[^67]: NetworkX has no dedicated
    [`skeleton()`](https://caugi.org/reference/skeleton.md) function.
    The moral graph (`nx.moral_graph`) is available, but skeleton
    extraction (dropping directionality without moralization) is not an
    exported function.
...
[^69]: No [`skeleton()`](https://caugi.org/reference/skeleton.md) method
    exists in any pgmpy graph class. The undirected skeleton is
...
[^72]: No dedicated
    [`skeleton()`](https://caugi.org/reference/skeleton.md) method.
    `GraphUtils.undirectedGraph(g)` converts all edges to undirected
...
[^82]: [`to_dot()`](https://caugi.org/reference/to_dot.md) /
    [`write_dot()`](https://caugi.org/reference/write_dot.md) write
    Graphviz DOT format, but caugi has no DOT reader—round-trip via DOT
    is not supported.

[^83]: [`caugi_serialize()`](https://caugi.org/reference/caugi_serialize.md)
    /
    [`caugi_deserialize()`](https://caugi.org/reference/caugi_deserialize.md)
    (and [`read_caugi()`](https://caugi.org/reference/read_caugi.md) /
    [`write_caugi()`](https://caugi.org/reference/write_caugi.md)) read
    and write caugi’s own JSON-based serialization format (Rust-side:
    `src/rust/src/lib.rs`). It is not a standardized cross-package JSON
    schema.
...
[^95]: `DAG.to_graphviz()` returns a `pygraphviz.AGraph` from which DOT
    ...
    [`to_dot()`](https://caugi.org/reference/to_dot.md) / `from_dot()`
    API; reading DOT is not supported through pgmpy’s own API.
...
[^102]: `pcAlgo` and `fciAlgo` objects have
    [`plot()`](https://caugi.org/reference/plot.caugi.md) S4 methods
    that delegate to `Rgraphviz`. `iplotPC()` delegates to `igraph`’s
    plot. `plotSG()` also requires `Rgraphviz`. There are no native
    layout algorithms in pcalg itself.
...
[^113]: `DAG.to_graphviz()` returns a pygraphviz `AGraph` (requires the
    ...
    [`plot()`](https://caugi.org/reference/plot.caugi.md) method that
    renders to screen directly. No built-in `matplotlib`-based
    [`plot()`](https://caugi.org/reference/plot.caugi.md) is present.
