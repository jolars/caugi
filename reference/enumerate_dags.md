...
[`meek_closure()`](https://caugi.org/reference/meek_closure.md) so any
background-knowledge orientations are preserved and propagated before
enumeration. The number of returned DAGs can grow super-exponentially in
the size of the chain components; use
[`count_dags()`](https://caugi.org/reference/count_dags.md) first to
size the problem.
