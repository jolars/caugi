...
- ...:
  ...
  retrieve. Multiple unnamed arguments drill down through nested
  options. To query all options, call without arguments. Attempting to
  access a non-existent option will raise an error.
...
[`posteriors()`](https://caugi.org/reference/posteriors.md),and
[`anteriors()`](https://caugi.org/reference/anteriors.md) exclude the
queried node itself. When `FALSE` (closed definition), the queried node
