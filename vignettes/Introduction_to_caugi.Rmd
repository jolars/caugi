---
title: "Get Starteded with `caugi`"
...
Create graphs from edge expressions and optional nodes. The graph can be **simple** (no parallel edges or self‑loops) and can carry a **class** label: `"Unknown"`, `"DAG"`, or `"PDAG"`. We plan on supporting several other causal graph types in future releases.
...
# add edges via expressions or vectors
cg <- add_edges(cg, E %-->% F)
```
...
* Unknown nodes trigger clear errors.
* For `simple = TRUE`, parallel edges or self‑loops are rejected.
* Internal R–Rust consistency is validated when built.
...
cg <- add_edges(cg, D %-->% G)
cg <- set_edges(cg, E %<->% F)
cg <- remove_nodes(cg, C)

# force build and check
cg <- build(cg)
is_dag(cg, force_check = TRUE)

# subgraph
sub <- subgraph(cg, A + B + D + G)
print(sub)
```

## Next steps

* Register custom edges with `register_caugi_edge()`.
* Use higher‑level algorithms as they land in future releases.
