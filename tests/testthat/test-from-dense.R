...
test_that("caugi_from_dense handles simple DAG", {
  m <- matrix(0L, 3, 3,
...
test_that("caugi_from_dense handles CPDAG (directed + undirected)", {
  m <- matrix(0L, 4, 4,
...
test_that("caugi_from_dense handles throws error given wrong edge types", {
  m <- matrix(0L, 5, 5,
    dimnames = list(LETTERS[1:5], LETTERS[1:5])
  )
  m["A", "B"] <- 1L # -->
